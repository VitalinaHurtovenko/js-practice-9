...
function findElementGreaterThan(arr, num) {
  ...
  let result = arr.find((elem) => (elem > num ? elem : false));
  return result;
...
function findIndexByProperty(arr, name) {
  ...
  let result = arr.findIndex((elem) => elem.name === name);
...
function getPositiveArrayValues(arr) {
  ...
  let zhnach = arr.values();
  // Конвертуємо ітератор в масив.
  let newArr = Array.from(zhnach);
...
function sumAndPrint(arr) {
  ...
  let elements = arr.forEach((elem) => console.log(elem));
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  let mappedArr = newArr.map((elem) => elem * elem);
  // За допомогою методу filter() фільтруємо елементи, які більше за порогове значення.
  let filteredArr = mappedArr.filter((elem) => elem > threshold);
...
function expandArrayByN(arr, n) {
  ...
  let newArr = arr.flatMap((elem) => Array(n).fill(elem));
...
function findLongestWord(arr) {
  ...
  let newWorld = arr.reduce((accumulator, currentValue) =>
...
function capitalizeWords(arr) {
  ...
  let newArr = arr.map((elem) => elem.charAt(0).toUpperCase() + elem.slice(1));
...
// Завдання: 19.
//ЗВЕРНІТЬ УВАГУ, метод findLastIndex працює на версії ноди вище 18.0.0 якщо ваша версія нижче, завдання можно не виконувати
// щоб перевірити версію ноди виконайте в терміналі команду node -v
